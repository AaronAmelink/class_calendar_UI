...
export default function SemesterCard({semester, classes, handleClick}) {
    ...
    return(
        <Card sx={{height:1, minWidth:1/5, border:1, borderRadius:'5%', borderColor: 'underline.secondary', overflow:'auto'}}>
            <CardContent>
                <Typography variant='h4' sx={{pt:2}}>{ semester }</Typography>
                <Typography variant='subtitle1' sx={{pt:2}}>{ creditTotal ? 'Total Credits: ' + creditTotal : '' }</Typography>
                <Divider/>
                <Stack>
                          ...
                          sx={{overflow: 'auto', my:2}}>
                        {
                            classes.map(classItem => {
                                if (classItem.planned) {
                                    return(<ClassChip code={classItem.courseCode} id={classItem.id} handleClick={handleClick} />);
                                }
                            })
                        }
                    </Stack>
